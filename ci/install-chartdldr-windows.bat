...
tasklist /FI "IMAGENAME eq opencpn.exe" 2>nul | %SystemRoot%\System32\find.exe /I "opencpn.exe" >nul
if %ERRORLEVEL%==0 (
...
if exist "%LIB_DST%" (
  set "STAMP="
  for /f "delims=" %%t in ('powershell.exe -NoProfile -Command "(Get-Date).ToString('yyyyMMddHHmmss')"') do set "STAMP=%%t"
